...
## Current state (2026-09-18, 23:15 SGT)
...
- Installer (superseded the first version the same night): the `installer\` folder is the complete distributable. `Build-Package.ps1` writes `strait-watch.zip`, `strait-share-<date>.db` and `VERSION.txt` into it (all three gitignored). `INSTALL.cmd` self-elevates, unpacks to `C:\claudespace\strait-watch`, installs pwsh 7 via winget if missing, runs `setup.ps1 -Unattended`, copies an optional `.env` in and the snapshot into `data\backups\` so Restore lists it, opens Settings. Built and parse-checked here; not yet run on a second PC.
- Map trails and a 30 s map refresh added after the installer work; same restart brings them live.
- Everything from the Backup tab onwards is committed and in the installer zip but NOT yet running on the service: one restart covers all of it.
- After the restart, expect on Settings: values imported from the old `.env`. Then set a backup destination, press Weekly now on Briefing.
...
- Everything is served locally (Chart.js, htmx, Leaflet vendored under `app/web/static`, behind basic auth). Map basemaps: Esri Dark Gray Canvas with reference labels (default), Light Gray, Ocean with labels, OpenStreetMap, switchable in the map's layer control; basemap, layer checkboxes and the 90-day toggle are remembered in localStorage; all free with attribution. CARTO tiles need a key now.
...
- Trails (2026-09-18): `ais_tracks` stores a point per vessel at most every 30 s (`TRACK_SECONDS`), `adsb_tracks` one per aircraft per poll, both pruned to 12 h (`TRACK_HOURS`); the latest-position tables keep 7 days. `/api/map?trail=<minutes>` returns one line per vessel or aircraft that is currently displayed, clamped to 12 h; 0 means the layer is off. Measured: ship points arrive about every 1 to 11 minutes (feed-limited, not our gate), 12 h payload about 125 KB, database 7.4 MB. Map redraws every 30 s.
