...
RETENTION_DAYS = 7      # how long the latest position of an aircraft is kept
...
def run() -> str:
    ...
    with closing(db.connect()) as conn, conn:
        for a, military in [(a, False) for a in civil] + [(a, True) for a in mil]:
            upsert(conn, a, military, ts)
            # Every poll is a track point, which is what the short trails on the map are drawn from
        ...
        conn.execute("DELETE FROM adsb_tracks WHERE ts_utc < ?", ((now - timedelta(hours=TRACK_HOURS)).isoformat(timespec="seconds"),))
        conn.execute("DELETE FROM adsb_aircraft WHERE ts_utc < ?", ((now - timedelta(days=RETENTION_DAYS)).isoformat(timespec="seconds"),))
