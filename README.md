...
## Map
- `/map`: Leaflet (vendored) with a basemap switcher (top right): Esri Dark Gray with city labels (default), Esri Light Gray, Esri Ocean with labels, OpenStreetMap. All free, no key, attribution shown. Basemap choice, layer checkboxes and the 90-day toggle are remembered per browser. Layers: MSA closure zones (polygons parsed from the Chinese warning detail pages by the `msa_zones` job, colored by kind), Japan Joint Staff and Coast Guard notices placed on the named passage or island, coast guard and naval ships, tankers, other ships (AIS, last 60 minutes), military and civil aircraft (ADS-B, last 20 minutes), and trails. Day picker or all 90 days. Refreshes every 30 seconds.
- Trails: every stored position of the ships and aircraft currently shown, over a window you pick (10 min, 30 min, 1 hour, 6 hours, 12 hours), coloured like the marker. Ship points are stored at most every 30 seconds per vessel, aircraft points once per poll, and 12 hours of points are kept. In practice the AIS feed hears a given ship every few minutes, so short ship trails are sparse; aircraft trails are smooth.
- `ais` runs as a background thread on the aisstream.io websocket (key on the Settings page; without it the job reports "not set" and the layer stays empty). Box 20.5 to 27.5 N, 115.5 to 123.5 E. Keeps the latest position per vessel (7 days), track points for all vessels (12 hours), and per-day sightings in named areas (Kinmen, Matsu, Taiwan ports, the Strait box).
- `adsb` polls adsb.lol every 2 minutes: civil aircraft within 250 nm of the Strait and all military aircraft in the region. Every poll is a track point (12 hours kept); latest positions 7 days, counts 90 days.
...
| `/map` | Basemap switcher, closure zones, notices, ships, aircraft, trails, day picker | 30 s |
